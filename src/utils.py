...
def show_random_solutions(solutions):
    print(
        f"Mostrando soluciones aleatorias (total: {len(solutions)}). Presiona Enter para mostrar la siguiente, o Ctrl+C para salir.\n"
    )
    try:
        while True:
            ...
            print(
                "\n".join(
                    " ".join("Q" if cell else "." for cell in row) for row in solution
                )
            )
            print("\n---\n")
